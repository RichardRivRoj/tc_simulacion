    ...
    return Div(
        Div(  # Contenedor para el scroll
            Table(
                Thead(
                    Tr(
                        *[Th(header, cls="text-center px-6 py-3 border-b border-gray-300 bg-gray-100 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider") for header in headers]
                    )
                ),
                Tbody(
                    *[
                        Tr(
                            *[Td(f"{fila[columna]}", cls="text-center px-6 py-4 border-b border-gray-200 text-sm text-gray-900") for columna in fila],
