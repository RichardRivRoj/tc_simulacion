...
def calc_mmk():
    ...
    form = card(
        Form(
            ...
            Label("K Número de servidores: ", Input(name="v_k", type="number", cls="flex rounded-md border mt-2 p-2", min=1, required=True), cls="block font-medium text-black text-lg mx-8"),
            Button("Calcular", type="submit", hx_post='/result_mmk', hx_target='#response', cls="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg px-5 py-2.5 text-center"),
            action = '/result_mmk',
            method="POST",
            cls="flex flex-col items-center gap-4 p-4"
        ),
        title="Calcular M/M/K",
        c="max-h-1/2"
    )
